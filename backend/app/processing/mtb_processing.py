...
# “Clean flight”: 2nd-order Butterworth LPF on raw G_total; takeoff peaks stay on raw; landing uses sustained G (10 Hz LPF + 100 ms mean).
...
# Landing / impact: 10 Hz LPF on ‖total acc‖/g (hand-damped vs mount chatter), then max of ~100 ms moving average in the post-flight landing window — no hard cap.
AIRTIME_LANDING_LPF_HZ = 10.0
LANDING_IMPACT_ROLLING_S = 0.100
...
def sustained_landing_g_series(g_total_g: np.ndarray, fs_hz: float) -> np.ndarray:
    """‖total acc‖/g → AIRTIME_LANDING_LPF_HZ Butterworth LPF, then centered rolling mean over LANDING_IMPACT_ROLLING_S."""
...
def apply_mtb_features(
    ...
    *,
    ...
    stats: dict[str, Any] = {
        ...
        # Max sustained landing ‖total a‖/g: find_peaks fallback while moving; replaced by validated-window max when > 0.
    ...
    # Landing G-Force fallback: prominence peaks on sustained ‖a‖/g while moving. Used when no validated airtime
    # segments exist; overwritten below by max from post-flight landing windows when sandwich validation passes.
    ...
    # Airtime: 10 Hz LPF on raw G_total for flight (20 Hz reference for diagnostics); sustained landing LPF + rolling mean for gate/stat.
    if peak_g is not None:
        ...
        if log_airtime_rejections:
            _log.info(
                ...
                "flight<%.2fg; min duration %.2fs; landing sustained (%.0fHz+%.0fms mean) >%.2fg within %.0fms; speed>%.1fm/s",
                _diag,
                ...
                LANDING_IMPACT_ROLLING_S * 1000.0,
                LANDING_PEAK_MIN_G,
        ...
        for a, b in _contiguous_true_runs(candidate):
            ...
            if not np.isfinite(landing_max) or landing_max < LANDING_PEAK_MIN_G:
                ...
                if log_airtime_rejections:
                    ...
                    _log.info(
                        ...
                        "Raw Peak: %.1fg | Sustained (%.0fms) Peak: %.2fg (need >%.2fg) in first %.0fms after flight; "
            ...
            if log_airtime_rejections and np.isfinite(raw_max_win) and np.isfinite(landing_max):
                _log.info(
                    "[landing impact] %s | Raw Peak: %.1fg | Sustained (%.0fms) Peak: %.1fg",
                    _diag,
                    raw_max_win,
                    LANDING_IMPACT_ROLLING_S * 1000.0,
                    landing_max,
