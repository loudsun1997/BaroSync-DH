...
export const AIRTIME_TOOLTIP =
  'Total seconds in the air: 10 Hz Butterworth LPF on raw ‖TotalAcceleration‖/g for flight (< ~0.55 g; 20 Hz min logged on first-run rejects), min ~0.20 s; sustained landing max (10 Hz LPF + 100 ms rolling mean on ‖TotalAcceleration‖/g) > ~1.3 g within 500 ms. Jump if raw takeoff > ~1.15 g in ~200 ms before flight; else drop. GPS speed > ~0.5 m/s. First run: see backend log [airtime diagnostic].'
...
export const LANDING_IMPACT_TOOLTIP =
  'Landing G-Force (“hand-damped”): 10 Hz low-pass on ‖TotalAcceleration‖/g, then max of a 100 ms rolling mean—post-flight landing windows when airtime validates, else prominence peaks on the same smoothed signal. No fixed cap.'
